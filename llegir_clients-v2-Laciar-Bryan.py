...
while sortir == False:	
	...
	os.system('clear')
	print "OPCIONS \n 1- Llegir taula 'pedidos o clientes': \n 0- Sortir \n"
	...
	# Llegim la taula "pedidos"
		...
		try:	
				if 	nombretable = raw_input("Inserte nombre de tabla: ")
					cur.execute(sql);
					...
					for row in rows:
						...
						print(" {:^12} {} {:^6} {:^5} {:^5} {:^12} {:^10} {:^10}".format(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7]))
				...
				if 	nombretable = raw_input("Inserte nombre de tabla: ")
					...
					for row in rows:
						#print row[0],row[1],row[2],row[3]
						print(" {:^11} {:^19} {:^10} {:^17}".format(row[0], row[1], row[2], row[3]))
